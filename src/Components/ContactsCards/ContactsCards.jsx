...
export const ContactsCards = ({ cardData }) => {
  ...
  return (
    <div className={`${style.contactsCards} ${theme === 'light' ? style.dark : style.light}`}>
      <div className={`${style.contactsCardsTitle} ${theme === 'light' ? style.dark : style.light} `}>{cardData.title}</div>

      {cardData.content ? (
        <div className={`${style.contactsCardsContent} ${theme === 'light' ? style.dark : style.light}`}>
          {cardData.content}
        </div>
      ) : (
        isSocialMedia && (
            <div className={`${style.contactsCardsImages} ${theme === 'light' ? style.dark : style.light}`}>
              <a href="https://www.instagram.com/startainstitute?igsh=MTNraWR1cHh3MjdpaA==">
                {theme === 'light' ? <img src={instagramWhite} alt="instagram" /> : <img src={instagram} alt="instagram" />}
              </a>
              <a href="https://wa.me/+499999999999">
                {theme === 'light' ? <img src={whatsAppWhite} alt="whatsApp" /> : <img src={whatsapp} alt="whatsApp" />}
              </a>
          </div>
        )
      )}
